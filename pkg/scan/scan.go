...
// GetIpOpenPort 获取开放端口号. 返回 开放端口 和 所有端口
func (s *ScanIp) GetIpOpenPort(ip string, port string) ([]int, []int) {
	var (
		total     int
		pageCount int
		num       int
	...
	fmt.Println(fmt.Sprintf("【%v】需要扫描端口总数:%v 个，总协程:%v 个，并发:%v 个，超时:%d 毫秒", ip, total, pageCount, num, s.timeout))
